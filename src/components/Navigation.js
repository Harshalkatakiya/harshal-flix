        ...
        <>
            <div className='grid fixed top-0 w-full m-0 z-20 ring-2  ring-red-300 ring-opacity-50 ring-offset-0
             backdrop-blur-lg backdrop-saturate-200 bg-white  bg-opacity-60 '>
                <img src={logo} className='mx-auto w-[25vh] h-auto mt-2 animate-[logo-animation_1s_ease]' alt="HarshalFlix" />
            </div>
                    ...
                    <ul className='flex gap-x-5 gap-y-2 mt-2 justify-center flex-wrap'>
                        {menuList.map((curelem) => {
                            return (
                                ...
                                active:scale-95 active:transition-all' onClick={() => { filterItem(curelem) }}>{curelem}</button>
                                </li>
                            );
                        })}
                    </ul>
